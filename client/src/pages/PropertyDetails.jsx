      ...
      <div className="relative">
        ...
        <PageContainer className="pt-28 pb-28 md:pt-32 md:pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-stretch">
            {/* LEFT – details card (RESTYLED) */}
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-100 p-5 md:p-7 lg:p-8 self-start flex flex-col gap-5 md:gap-6">
              {/* MOBILE HEADER */}
              ...
              <div className="hidden md:flex justify-between gap-4">
                <div className="flex-1">
                  <h1 className="text-[28px] lg:text-[32px] font-semibold text-gray-900 leading-tight">
                    {property.title}
                  </h1>
              ...
              {/* PRICE + VERIFIED + QUICK STATS — responsive */}
                    ...
                    <div className="flex flex-col text-left">
                      ...
                      {propertyIdDisplay && (
                        <span className="text-[11px] text-gray-500">
                          Ref: {propertyIdDisplay}
                        </span>
                      )}
                    </div>
                  </div>

                  {/* Price block — LEFT aligned on mobile, RIGHT on desktop */}
              ...
              {baseFeatureChips.length > 0 && (
                <div className="w-full">
                  {/* Mobile: single row, no wrap (3 items share space equally). Desktop falls back to 3-col grid */}
                  <div className="flex items-center justify-between gap-2 flex-nowrap md:grid md:grid-cols-3 md:gap-0">
                    {baseFeatureChips.map((item, idx) => (
                      <div
                        key={item.id}
                        className={
                          // each chip takes equal space, can shrink; keep divider on md+
                          "flex-1 min-w-0 flex items-center gap-2 py-2 px-1 text-center " +
                          (idx > 0
                            ? "md:border-l md:border-gray-200 md:pl-6 md:text-left"
                            : "md:pl-0")
                        }
                      >
                        {/* compact icon on mobile, larger on md */}
                        <div className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-md bg-gray-100 text-gray-700 md:w-10 md:h-10">
                          {React.cloneElement(item.icon, {
                            className: "w-3.5 h-3.5 md:w-5 md:h-5",
                          })}
                        </div>

                        {/* label: small on mobile, truncates if too long */}
                        <div className="min-w-0">
                          <span className="block text-xs font-semibold text-gray-900 truncate md:text-lg">
                            {item.label}
                          </span>
                        </div>
                      </div>
              ...
              {/* Description – tighter, bigger text */}
              <div className="border-t border-gray-100 pt-4">
                <h2 className="text-base md:text-lg font-semibold text-gray-900 mb-1.5">
                  Description
                </h2>
              ...
              {/* Key Features – compact, more dense */}
                ...
                <div className="border-t border-gray-100 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {amenityChips.map((item) => (
                      <div
                        key={item.id}
                        className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs md:text-sm text-gray-800"
                      >
              ...
              {/* AREAS & LOT – tighter rows, bigger labels */}
              <div className="border-t border-gray-100 pt-4">
                <h2 className="text-base font-semibold text-gray-900 mb-2">
                  Areas &amp; Lot
                </h2>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2.5 text-xs md:text-sm">
              ...
              {/* Bank info block – kept but tightened slightly */}
              <div className="mt-2 bg-[#EDEAE3] rounded-xl p-4 md:p-5 shadow-sm">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-2">
                  {[Bank1, Bank2, Bank3, Bank4, Bank5, Bank6].map((bank, i) => (
                    <img
                      ...
                      src={bank}
                      alt={`Bank ${i + 1}`}
                      className="h-10 md:h-12 w-auto object-contain opacity-90 hover:opacity-100 transition duration-200"
                    />
                  ))}
                  <span className="text-gray-800 text-xs md:text-sm font-medium italic">
            ...
            {/* RIGHT – Enquiry + Nearby (unchanged) */}
                            ...
                            return (
                              <div key={relId} className="px-1">
                                <article
                                  ...
                                  className="rounded-xl border border-gray-200 bg-white cursor-pointer shadow-sm hover:shadow-md transition overflow-hidden"
                                >
                                  {/* IMAGE WITH PADDING */}
                                  <div className="p-3">
                                  ...
                                  {/* ====== Price Left — View Button Right ====== */}
                                  <div className="px-3 py-3 flex items-center justify-between">
                                    <div className="text-sm font-semibold text-gray-900">
                                      ₹ {Number(relPrice).toLocaleString("en-IN")}
                                    </div>
                                    ...
                                    <button
                                      ...
                                      className="px-3 py-1.5 rounded-full bg-black text-white text-xs font-medium hover:bg-gray-800 transition"
                                    >
                                      View
                                    </button>
                                  </div>
    ...
    {related.length ? (
      <div className="space-y-6">
        {related.map((p) => {
          ...
          const relPrice = p.totalPrice ?? p.price ?? 0;
          const imgSrc =
            (p.images && p.images.length && p.images[0]) || fallbackImg;

          return (
            <article
              onClick={() => navigate(`/property/${relId}`)}
              key={relId}
              // FIX 1: Tighter padding and gap at lg (1024px) breakpoint
              className="flex items-stretch gap-3 lg:gap-2 xl:gap-3 p-3 lg:p-2 xl:p-3 rounded-lg border border-gray-100 cursor-pointer hover:shadow-lg transition-transform transform hover:-translate-y-1 bg-white min-w-0"
            >
              <img
                src={imgSrc}
                alt={p.title}
                // FIX 2: Slightly smaller image at lg to give text more room
                className="w-24 h-20 lg:w-20 lg:h-20 xl:w-24 xl:h-20 object-cover rounded-md flex-shrink-0"
              />

              <div className="flex-1 min-w-0 flex flex-col">
                <div className="flex items-start justify-between">
                  <h4 className="text-sm font-semibold text-gray-900 line-clamp-2 truncate min-w-0">
                    {p.title}
                  </h4>
                </div>

                <div className="flex items-center justify-between mt-2">
                  <p className="text-xs text-gray-600 mt-1 truncate min-w-0">
                    {p.city}
                    {p.country ? `, ${p.country}` : ""}
                  </p>
                ...
                {/* FIX 3: Added flex-wrap and gap-y to handle overflow gracefully */}
                <div className="mt-2 flex flex-wrap gap-y-1 items-center justify-between">
                  <div className="flex items-center gap-4 lg:gap-2 xl:gap-4 text-xs text-gray-600">
                    <span className="flex items-center gap-1">
                      <Bed className="w-4 h-4" /> {p.rooms ?? "-"}
                    </span>
                    <span className="flex items-center gap-1">
                      <Bath className="w-4 h-4" /> {p.bathrooms ?? "-"}
                    </span>
                    <span className="flex items-center gap-1">
                      <Expand className="w-4 h-4" />{" "}
                      {p.squareMeters ?? p.size ?? "-"}{" "}
                      <span className="hidden xl:inline">m²</span>
                    </span>
                  </div>

                  {/* FIX 4: Hide the button on lg (1024px) where space is tight, show on xl+ */}
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/property/${relId}`);
                    }}
                    className="px-3 py-1 rounded-full bg-black text-white text-sm font-medium hover:bg-gray-800 transition cursor-pointer lg:hidden xl:block"
                  >
                    View
                  </button>
                </div>
