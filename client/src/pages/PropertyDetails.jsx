...
const PropertyDetails = () => {
 ...
 const FIXED_AGENT_PHONE = "+91 95003 06566";
      ...
      try {
        ...
        try {
          ...
          const relatedProps = all
            ...
            .slice(0, 6); // fetch up to 6 so we can show more in sidebar rows
  ...
  // base stat chips (beds/baths/area)
  ...
  // convert amenities array to chips with icons (defensive & flexible)
  const rawAmenities = property.amenities || [];

  const amenityChips = rawAmenities.map((a, idx) => {
    // Support both string and object-shaped amenities
    let label = "";
    ...
    if (key.includes("garden")) {
      ...
      // fallback: small dot if no icon defined
      icon = (
  ...
  const buildMobileWhatsAppMessage = () => {
    ...
    // include the current URL so the agent or recipient can open it
 ...
 const handleMobileWhatsApp = () => {
  ...
  return (
    <div className="relative">
      <div className="max-w-7xl mx-auto px-6 pt-28 pb-8 md:pt-32 md:pb-20">
        {/* increased bottom padding (pb-36) so mobile bottom bar doesn't overlap content */}
        ...
        {/* 🖼️ MAIN + THUMBNAIL GALLERY */}
        {activeMainImage && (
          <div className="mb-10 mt-4">
            {/* Desktop / tablet: main left, thumbs right */}
                      ...
                      >
                        ...
                        {isLast && (
                          <div className="absolute inset-0 bg-black/55 flex items-center justify-center">
                            <span className="text-white font-semibold text-lg">+{extraDesktopCount}</span>
                          </div>
                        )}
                      </button>
                    );
            ...
            {/* Mobile: big image + horizontal thumbs below */}
                  ...
                  {images.map((img, i) => (
                    <button
                      ...
                      className={`flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all duration-200 cursor-pointer ${
                        activeMainImage === img ? "border-black" : "border-transparent"
                      }`}
                    >
                      <img src={img} alt={`Property ${i + 1}`} className="w-24 h-20 object-cover" />
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-stretch">
          {/* LEFT – details card like the reference design */}
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 md:p-8 self-start flex flex-col justify-between h-full ">
            {/* Top row: title + share icons on right */}
            <div className="flex items-start justify-between gap-4 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 16 16" className="text-green-600" fill="currentColor">
                ...
                <button
                  ...
                  aria-label="Share"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4 h-4 text-gray-700" fill="none" stroke="currentColor" strokeWidth="1.5">
            ...
            {/* Verified + Price row (no ref under location) */}
              ...
              <div className="p-0 md:p-1">
                <div className="flex flex-wrap gap-2 md:gap-3">
                  {keyFeatureChips.map((item) => (
                    <div key={item.id} className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs md:text-sm text-gray-700">
                      <span className="text-gray-500 flex items-center justify-center">{item.icon}</span>
            ...
            {/* AREAS & LOT */}
            <div className="mt-8">
              <h2 className="text-sm font-semibold text-gray-700 mb-2">Areas &amp; Lot</h2>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
                <div className="flex items-center justify-between px-4 py-3 text-xs md:text-sm">
                  ...
                  <span className="font-medium text-gray-900">{property.status || "For Sale"}</span>
                </div>
                ...
                <div className="flex items-center justify-between px-4 py-3 text-xs md:text-sm">
                  ...
                  <span className="font-medium text-gray-900">{size ? `${size} m²` : "N/A"}</span>
                </div>

                <div className="flex items-center justify-between px-4 py-3 text-xs md:text-sm">
                  <span className="text-gray-500">Property Type</span>
                  <span className="font-medium text-gray-900">{property.type || "N/A"}</span>
                </div>

                <div className="flex items-center justify-between px-4 py-3 text-xs md:text-sm">
                  <span className="text-gray-500">Reference ID</span>
                  <span className="font-medium text-gray-900">{propertyIdDisplay || "–"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 bg-[#EDEAE3] rounded-xl p-6 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-6 mt-4">
                {[Bank1, Bank2, Bank3, Bank4, Bank5, Bank6].map((bank, i) => (
                  <img key={i} src={bank} alt={`Bank ${i + 1}`} className="h-12 w-auto object-contain opacity-90 hover:opacity-100 transition duration-200" />
                ))}
          ...
          {/* RIGHT – Enquiry + "You May Also Like" moved under the enquiry panel */}
          <div className="space-y-6 flex flex-col justify-between h-full">
            <div>
              <EnquiryFormPanel propertyTitle={property.title} propertyRef={propertyIdDisplay} agentPhone={agentPhone} />
            </div>

            {/* --- PROPERTIES NEARBY (MOBILE SLIDER + DESKTOP LIST) --- */}
            {(() => {
              const fallbackImg = "/mnt/data/bc98def2-2ad1-4314-8d62-eb32ff8eda97.jfif";
              ...
              return (
                <div className="p-4 ">
                  <h3 className="text-lg font-semibold text-gray-900 mb-3">Properties Nearby</h3>

                  {/* MOBILE: slider - SAME horizontal card design */}
                  <div className="md:hidden">
                    {related.length ? (
                        ...
                        {related.map((p) => {
                          ...
                          return (
                            <div key={relId} className="px-1">
                              <article onClick={() => navigate(`/property/${relId}`)} className="flex items-stretch gap-3 p-3 mb-3 rounded-lg border border-gray-200 bg-white cursor-pointer shadow-sm hover:shadow-md transition-transform transform hover:-translate-y-1">
                                {/* thumbnail */}
                                <img
                                  src={imgSrc}
                                  alt={p.title}
                                ...
                                <div className="flex-1 min-w-0 flex flex-col">
                                  ...
                                  <div className="mt-2 flex items-center justify-between">
                                    ...
                                    <button
                                      ...
                                      className="px-3 py-1 rounded-full bg-black text-white text-xs font-medium hover:bg-gray-800 transition"
                                    >
                                      View
                                    </button>
                                  </div>
